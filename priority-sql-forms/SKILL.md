...
description: >
  ...
  tables), inspecting form/column structure via the Priority REST API
  (EFORM endpoint, FCLMN_SUBFORM, FLINK_SUBFORM), and creating a form
...
no-direct-write-to-standard-table rule (§3, and the `priority-sql` skill).
...
keys are the real field `NAME`s. See the `priority-sql` skill for the standard
